...
fig, axs = visualizer.spectrogram_display(db, start=start, end=end, time_format="seconds", section="all", showscale="True", sensor="ASPIDS", compressed=True)
for ax in axs:
    ax.set_ylim(0, 8000)
...
log = db.session.get(spidb.Log, 239)
start = log.start + dt.timedelta(seconds=0)
end = start + dt.timedelta(seconds=60)

fig, ax = visualizer.spectrogram_display(db, start=start, end=end, sensor="MSPIDS", time_format="seconds", section="all", showscale="True", compressed=True, zmin=-100, zmax=-50)

# fig.savefig(rf"reports/documents/NeurIPS_SPID/images/microwave_spectrogram_display_{log.target}_{log.material}.pdf", dpi=300, bbox_inches="tight")
#%%